...
| # | Item | Status | Notes |
|---|------|--------|-------|
| A-1 | Options node architecture | Decided | Per-provider Options nodes with toggles. API-4 research confirmed parameter names differ enough across backends that adapter mapping handles translation naturally. No chaining, no shared base node. Each Options node shows only its backend's supported parameters. |
...
| A-3 | Per-parameter enable/disable toggles | Decided | Yes, toggles. BOOLEAN widget confirmed reliable (P-9). Disabled = model default, enabled = override. Each Options node uses toggle+value pairs for its parameters. |
