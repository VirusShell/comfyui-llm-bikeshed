...
| A-16 | Supported local backends | Decided | **In-pack:** LM Studio, text-generation-webui, OpenAI Chat Completions, and generic OAI-compat hosts **including llama.cpp `/v1`** (same adapter). **Removed (D-3):** native Ollama. Dropped vLLM-specific nodes and a dedicated llama-server node (see D-4). Indirect llama.cpp via LM Studio / Textgen unchanged. |
...
| P-11 | Provider `url` change → model dropdown refresh (multi-provider graphs) | Decided | **Fixed 2026-06-17:** `js/model_dropdown.js` reads `urlWidget.value` in debounced refetch (not callback arg); DOM `input`/`change` listeners on URL widget as backup; per-node handlers; `loadedGraphNode` re-schedules fetch after workflow restore. Empirical bug: [`user-feedback-2026-06-17.md`](research/user-feedback-2026-06-17.md). Re-verify in live ComfyUI with OAI + Textgen on one graph. |
| P-12 | Provider status widgets (`detected_backend`, `loaded_model_status`) read-only UX | Decided | **Fixed 2026-06-17:** JS status widgets use `{ serialize: false, disabled: true, read_only: true }` (ComfyUI frontend ~1.39+ Vue renderer). Rename/clarity (`loaded_model` label) remains optional polish. |
...
| D-4 | Dedicated llama.cpp / llama-server **node** | Tabled | Dedicated provider/lifecycle/process-manager node still deferred. **Clarified 2026-09-14:** llama.cpp via **OAI Compatible** (`/v1`) is first-class (detection/docs/UX/presets); see product-direction. Indirect use via LM Studio / Textgen unchanged. |
...
| 2026-09-14 | Clarified D-4 / A-16: dedicated llama-server node remains tabled; OAI Compatible path for llama.cpp is first-class per product-direction. |
...
| 2026-06-17 | **P-11 / P-12 fixed:** `model_dropdown.js` URL refetch uses widget value + DOM listeners + `loadedGraphNode`; status widgets `disabled`/`read_only`. |
