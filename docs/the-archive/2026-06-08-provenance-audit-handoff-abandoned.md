# Project-wide provenance audit — operational handoff (archived)

> **Status: ABANDONED** (2026-06-11). Preserved for historical context only. Do not treat as active work. For ongoing rules, use [`provenance-and-reverification.md`](../research/provenance-and-reverification.md) and [`fresh-context-prevention-prompt.md`](../research/fresh-context-prevention-prompt.md).

**Created:** 2026-06-07  
**Last updated:** 2026-06-08 (backward inventory deferred; prevention authoritative)  
**Archived:** 2026-06-11 — systematic repo-wide sweep cancelled; branch `research/audit-and-cancel-tracking` not merged to `master`.  
**Context:** Assessment `f0aad497` found a gap — fresh agents need a single kickoff doc, not chat history.  
**Fresh chat (default):** paste [`fresh-context-prevention-prompt.md`](fresh-context-prevention-prompt.md). **Backward inventory only:** [`fresh-context-audit-prompt.md`](fresh-context-audit-prompt.md).

> **Priority note:** This document is **secondary / deferred**. For new work, bugs, and features, **decision-time prevention** in [`provenance-and-reverification.md`](provenance-and-reverification.md) and [`research-note-template.md`](research-note-template.md) is authoritative. The checklist below remains useful for release planning or explicit repo-wide sweeps — not as the default session entry point.
...
**New work / default:** [`provenance-and-reverification.md`](provenance-and-reverification.md) → [`research-note-template.md`](research-note-template.md) → [`resolution_tracker.md`](../resolution_tracker.md) § Status definitions.

**Backward inventory (this doc):**

1. **This document** — Tier 1 checklist, next work
2. [`provenance-and-reverification.md`](provenance-and-reverification.md) — decision-time gates (authoritative for cites going forward)
...
| **Remote** | Private Gitea instance (redacted) — migrated to GitHub 2026-06 |
| **Base branch** | `master` |
...
|---|------|------------|-----------|--------|
| 1.1 | **Resolution tracker contamination audit** | For each `Decided`, `Assumed`, `Confirmed`, and any `Contaminated` row: list external citations in Notes; check source date + status; downgrade or flag if evidence is docs-only, undated, or stale | `docs/resolution_tracker.md` | **Partial** — high-impact rows reviewed 2026-06-08: A-22 (#1463 still open), API-7 (startup key copy clarified); A-15/A-18/A-19 empirical `[VERIFY]` retained |
| 1.2 | **Research claim inventory** | Build/update provenance tables in each `docs/research/*.md`: URL, source date, access date, verified how, applies-to | `docs/research/*.md` | **Partial** — `textgen-lifecycle-verified.md`, `lm-studio-lifecycle-verified.md` match template; `cancel-interrupt-status.md` substantive (no full template); handoff/meta docs use prevention pointers |
| 1.3 | **Reference docs vs code** | Diff allowlists, routes, auth, platform patterns against adapters and nodes | `docs/reference/backend-api-parameters.md`, `implementation-patterns.md`, `comfyui-platform-findings.md` vs `adapters/`, `nodes/` | **Partial** — `backend-api-parameters.md` Textgen auth + Ollama historical note fixed 2026-06-08; `implementation-patterns.md` PROMPT hidden key + `has_downstream_gen_node` aligned; stale PromptServer example still shows Ollama route **[VERIFY]** if vendored as copy-paste |
| 1.4 | **Concept & scope vs code** | Reconcile `text_gen_processing_concept.md`, `CLAUDE.md`, S-2/v1 scope with shipped nodes and adapters | `docs/text_gen_processing_concept.md`, `CLAUDE.md`, `docs/proposals/product-direction-and-scope.md` | **Partial** — `CLAUDE.md` + `product-direction-and-scope.md` updated to shipped v1; concept doc not fully reconciled |
...
Lower urgency unless shipping that work. See `provenance-and-reverification.md` § 9 (backward audit tiers).
...
| **2** | **Research `*.md` claim inventory** — add/fix provenance tables; mark stale claims | 1.2 | Copy [`research-note-template.md`](research-note-template.md); gold example: `textgen-lifecycle-verified.md` |
...
| 2026-06-08 | Prevention follow-ups + Tier 1 pragmatic sweep | Gold research notes backfilled; [`fresh-context-prevention-prompt.md`](fresh-context-prevention-prompt.md) created; tracker/reference/CLAUDE fixes; Tier 1 rows 1.1–1.5, 1.7–1.8 → Partial/Verified where touched |
...
| Doc | Role |
|-----|------|
| [provenance-and-reverification.md](provenance-and-reverification.md) | Decision-time gates (authoritative for new work) |
| [research-note-template.md](research-note-template.md) | Copy-paste structure for new research notes |
| [fresh-context-prevention-prompt.md](fresh-context-prevention-prompt.md) | **Default** paste for feature/bug work (decision-time gates) |
| [fresh-context-audit-prompt.md](fresh-context-audit-prompt.md) | Optional paste for backward inventory sessions |
